...
template<typename T>
inline T bilint_one(int const ny, int const nx, const T* const __restrict__ im, T  yy, T  xx)
{

  yy = std::min<T>(std::max<T>(T(0),yy), T(ny-1));
  xx = std::min<T>(std::max<T>(T(0),xx), T(nx-1));

  int const ix = std::min<int>(int(xx), nx-2);
  int const iy = std::min<int>(int(yy), ny-2);

  
  double const dx = xx-ix;
  double const dx1= 1.0 - dx;
  double const dy = yy-iy;
  double const dy1= 1.0 - dy;

  return T(im[iy*nx+ix]*dx1*dy1 + im[iy*nx+ix+1]*dx*dy1 + im[(iy+1)*nx+ix]*dx1*dy + im[(iy+1)*nx+ix+1]*dx*dy);
}
...
#pragma omp parallel default(shared) firstprivate(jj) num_threads(nthreads)
  {
#pragma omp for schedule(static)
    for( jj=0; jj<ntot; ++jj){
      res_in[jj] = bilint_one<double>(ny, nx, d_in, yy_in[jj], xx_in[jj]);
